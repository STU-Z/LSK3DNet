...
def transform_points(points, T):
    ...
    points[:, :3] = xyz1[:, :3]
    return points
def gather_spatio_temporal_data(data_dicts):
...
if __name__ == "__main__":
    ...
    bin_path = "/media/zrb/Zrb-TB2/kitti/SemanticKITTI_Data/SemanticKitti/sequences/00/velodyne/000005.bin"
    ...
    data_dict = load_semkitti_bin(
        bin_path, learning_map, imageset='train', length=3)
    ...
    # 可视化实例标签
    instance_color_map = get_instance_color_map(merged_instance_labels)
    colors_instance = np.array([instance_color_map[inst] for inst in merged_instance_labels])
    pcd_instance = o3d.geometry.PointCloud()
    pcd_instance.points = o3d.utility.Vector3dVector(merged_points[:, :3])
    pcd_instance.colors = o3d.utility.Vector3dVector(colors_instance)
