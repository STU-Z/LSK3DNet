...
def SNIP(net, keep_ratio, train_dataloader, device, masks, distributed):
    if distributed:
        train_dataloader.sampler.set_epoch(0)

    # Grab a single batch from the training dataset
    train_data_dict = next(iter(train_dataloader))
    train_data_dict['points'] = train_data_dict['points'].to(device, non_blocking=True)
    train_data_dict['normal'] = train_data_dict['normal'].to(device, non_blocking=True)
    train_data_dict['ref_xyz'] = train_data_dict['ref_xyz'].to(device, non_blocking=True)
    train_data_dict['batch_idx'] = train_data_dict['batch_idx'].to(device, non_blocking=True)
    train_data_dict['labels'] = train_data_dict['labels'].to(device, non_blocking=True)
    train_data_dict['raw_labels'] = train_data_dict['raw_labels'].to(device, non_blocking=True)
    train_data_dict['origin_len'] = train_data_dict['origin_len'].to(device, non_blocking=True)
    train_data_dict['indices'] = train_data_dict['indices'].to(device, non_blocking=True)

    # Let's create a fresh copy of the network so that we're not worried about
    # affecting the actual training-phase
    net = copy.deepcopy(net)
    net.zero_grad()
    outputs = net(train_data_dict)
    loss = F.cross_entropy(outputs['logits'], outputs['labels'].long())
    loss.backward()
...
class Masking(object):
    ...
    """
    def __init__(self, optimizer, prune_rate_decay, prune_rate=0.5, prune_mode='magnitude', 
                growth_mode='random',  redistribution_mode='momentum', fp16=False, update_frequency=None, 
                sparsity=None, sparse_init=None, device=None, distributed=False):
    ...
    def add_module(self, module):
        ...
        self.init(mode=self.sparse_init, density=1-self.sparsity)
    ...
    def init(self, mode='snip', density=0.05, erk_power_scale=1.0):
        ...
        if mode == 'uniform':
            ...
            self.baseline_nonzero = 0
            layer_wise_sparsities = SNIP(self.module, density, self.train_loader, self.device, self.masks, self.distributed)
