...
class point_encoder(nn.Module):
    ...
    def downsample(coors, p_fea, scale=2):
        ...
        # inv = torch.unique(torch.cat([batch, coors[:, [3, 2, 1]]], dim=1), return_inverse=True, dim=0)[1]  
        return torch_scatter.scatter_mean(p_fea, inv, dim=0), inv

    def forward(self, features, data_dict):
        output, inv = self.downsample(data_dict['coors'], features)  # problem
        # output, inv = self.downsample(data_dict['full_coors'], features) 
        # print("features.shape: ", features.shape,flush=True)
        # print("data_dict['coors'].shape: ", data_dict['coors'].shape,flush=True)
        ...
        # print("data_dict['full_coors'].shape: ", data_dict['full_coors'].shape,flush=True)
